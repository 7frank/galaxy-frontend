...
ConvexHullGrahamScan.prototype = {
	...
	addPoint3d: function (vec,camera) {
       
	   
		var pt=vec.clone().project(globalEnv.camera)
		pt.orig=vec
	   ...
	   //Check for a new anchor
        ...
        if ( newAnchor ) {
            if ( this.anchorPoint !== undefined ) {
				var _ap=new this.Point(this.anchorPoint.x, this.anchorPoint.y)
				_ap.orig=this.anchorPoint.orig
                this.points.push(_ap);
				
            }
            this.anchorPoint = pt// new this.Point(x, y);
        } else {
