...
let curr = 0;
let currentGradientColors = [0x218D20, 0x439229, 0x8CCB84, 0x14B0BF, 0x9DC9CA, 0xCAB81A, 0xBBC42D, 0xC8A6BF, 0xCF73B4, 0x816365, 0x7D5C53, 0xAE5E29, 0xB62729];

function getNextGradient(): number[] {
    const availGradients: number[][] = [
    ...
    const next = ++curr % availGradients.length;
    return currentGradientColors = availGradients[next];
}

export function computeCompanyNodeColor(val = 0, attr = "sent"): number {
    const sentRanges = [[92, Number.MAX_SAFE_INTEGER], [85, 92], [78, 85], [71, 78], [64, 71], [57, 64], [50, 57], [43, 50], [36, 43], [29, 36], [22, 29], [15, 22], [Number.MIN_SAFE_INTEGER, 22]];
    const priceRangesInPct = [[18, Number.MAX_SAFE_INTEGER], [18, 15], [12, 15], [9, 12], [6, 9], [3, 6], [0, 3], [-3, 0], [-6, -3], [-9, -6], [-12, -9], [-18, -15], [Number.MIN_SAFE_INTEGER, -18]];

    let arr: number[][];
    if (attr == "sent") arr = sentRanges;
    else if (attr == "priceRanges") arr = priceRangesInPct;
...
export function computeGroupNodeColorHelper(distinctGroupIDS: string[]): { getColor: (groupID: string) => number } {
    const colors: number[] = [];
    for (let i = 0; i < distinctGroupIDS.length; i++) {
