			...
			const loadCSV = function (Graph) {
				...
				Papa.parse(fileName, {
					...
					step: function(row) {
						...
						var data={id:r.id,name:r.name,group:r.country,industry:r.industry,sent:r.sent,price:r.price,itemCount:r.itemCount}
		...
		function streamCSVFile(fileName,onRowCallback,onComplete)
		{
			return new Promise(function(ok,fail){
				...
				Papa.parse(fileName, {
					...
					dynamicTyping: true,
					step: onRowCallback,
					complete: function (){  if (typeof onComplete=="function")onComplete();  ok()    }
				});
		...
		//the load function itself
			const loadCSV = function (Graph) {
				...
				var nodesPromise=streamCSVFile(fileName,function(row) {
						...
						var data={id:r.id,name:r.name,group:r.country,industry:r.industry,sent:r.sent,price:r.price,itemCount:r.itemCount,ticker:r.ticker}
